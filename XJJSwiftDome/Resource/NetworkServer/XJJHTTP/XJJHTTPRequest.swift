...
final class XJJHTTPRequest: NSObject {
    ...
    func request<T: Codable>(_ urlStr: String,
        ...
        guard let url = URL(string: url_str!) else {
            ...
            print("request base:", "-url-", "invalid url:", self.urlString ?? "no url")
            return
        }
    ...
    func download(_ urlStr: String,
        ...
        if let data = self.cache.object(forKey: urlStr as AnyObject) as? Data {
            ...
            print("download", "-has cache-", data.count)
            return
        }else {
            self.createDownload(urlStr, fileName, saveFileType: saveFileType, needCache: needCache) {[weak self] (isSuccess, filePath, data) in
                ...
                //ELog.request_log("request base:", "-download-", "Is main thread:", Thread.current.isMainThread)
            ...
            autoreleasepool {
                ...
                if let str = item.urlStr {
                    if let data = self.cache.object(forKey: str as AnyObject) as? Data {
                        ...
                        print("download", "-has cache-", "NO.\(i)", data.count)
                    }else {
                        self.createDownload(str, item.fileName ?? "file", saveFileType: item.saveFileType, needCache: item.needCache) {[weak self] (isSuccess, filePath, data) in
                            ...
                            //ELog.request_log("request base:", "-download-batch", "Is main thread:", Thread.current.isMainThread)
                            //ELog.request_log("request base:", "-download-taskCount", sself.downloadTask.items.count, sself.downloadTask.tempItems.count, sself.downloadTask.isRuning)
                            DispatchQueue.main.async {
    ...
    private func sendRequest<T: Codable>(emptyFormat: Bool? = false,
                                         callback: ((_ isSuccess: Bool,_ result: T?, _ d: String?, _ errorCode: Int?) -> Void)?) {
        //ELog.request_log("request base:", self.urlString ?? "", self.method, self.header, self.params)
        let urlStr = self.urlString ?? ""
                
        let task: URLSessionDataTask = self.session.dataTask(with: request) {[weak self] (data, response, error) in
            guard let sself = self else {return}
            print("response base:", response ?? (urlStr + "no response"))
            //ELog.request_log("current is main thread -- ", Thread.current == Thread.main)
                ...
                if let jsonData = data {
                    do { // 确定json格式处理
                        if emptyFormat == true {
                            ...
                            print("request base:", "-result-", jsonData.JSONToAny())
                            DispatchQueue.main.async {
                                callback?(true, result, "", 0)
                            }
                        }else {
                            let result = try JSONDecoder().decode(XJJHTTPResult<T>.self, from: jsonData)
                            print("request base:", "-result-", jsonData.JSONToAny(), "-result-T-", result)
                            DispatchQueue.main.async {
                                callback?(true, nil, "result", 0)
                            }
                        }
                    }catch { // 不确定json格式处理, 转成字符串查看数据
                        print("request base:", "-decode error-", urlStr, error)
                        DispatchQueue.main.async {
                            sself.jsonToStr(jsonData, callback: callback as? ((Bool, String?, String?, Int?) -> Void))
                        }
                    }
                }else { // 网络请求成功，数据返回失败
                    print("request base:", "-error-", urlStr, "success but no any data!")
                    DispatchQueue.main.async {
                        callback?(false, nil, nil, -2)
                    }
                }
            }else { // 网络请求失败
                print("request base:", "-error-", urlStr, error?.localizedDescription ?? "no value")
                DispatchQueue.main.async {
    ...
    private func jsonToStr(_ jsonData: Data,
        ...
        if resultStr.count > 0 { // 转字符串
            print("request base:", "-result-string", resultStr)
            callback?(false, resultStr, resultStr, -4)
        }else { // 转换失败
            print("request base:", "-error-", "data can't change to string type!")
    ...
    private func createDownload(_ urlStr: String,
        ...
        guard let url = URL(string: url_str!) else {
            print("request base:", "-download url-invalid url:", url_str ?? "no url")
            return
        }
        ...
        //ELog.request_log("request base:", "-download-create", "Is main thread:", Thread.current.isMainThread)
        ...
        if needCache == true {
            let task = self.session.dataTask(with: request) {[weak self] (data, response, error) in
                ...
                guard error == nil else {
                    ...
                    print("request base:", "-download-error:", error ?? "no error")
                    return
                }
                
                sself.cache.setObject(data as AnyObject, forKey: urlStr as AnyObject, cost: data?.count ?? 0)
                print("request base:", "-download-result:", data?.count ?? "0")
                callback?(true, nil, data)
            }
            item.dataTask = task
            item.task = nil
            ...
            task.setValue(item.identifier, forKey: "taskIdentifier")
            item.task = task
            item.dataTask = nil
    ...
    private func createUpload(_ uploadItem: XJJHTTPUploadWorkerItem,
        ...
        guard let urlStr = uploadItem.urlStr, let data = uploadItem.data else {
            print("request base:", "-upload url or data-invalid:", uploadItem.urlStr ?? "no url", uploadItem.data ?? "no data")
            return
        }
        let url_str = urlStr.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
        guard let url = URL(string: url_str!) else {
            print("request base:", "-upload url-invalid url:", url_str ?? "no url")
            return
        }
...
extension XJJHTTPRequest: URLSessionTaskDelegate, URLSessionDataDelegate {
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didSendBodyData bytesSent: Int64, totalBytesSent: Int64, totalBytesExpectedToSend: Int64) {
        ...
        for item in self.uploadTask.items {
            if item.task?.taskIdentifier == task.taskIdentifier {
                print("request base:", "-upload-send bytes:", bytesSent, "total send bytes:", totalBytesSent, "total bytes:", totalBytesExpectedToSend)
                
                let progress: Float = Float(totalBytesSent / totalBytesExpectedToSend)
                item.uploadProgressBlock?(progress)
                print("request base:", "-upload-progress:", progress)
            }
        }
    }
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        for item in self.uploadTask.items {
            if item.task?.taskIdentifier == task.taskIdentifier {
                print("request base:", "-upload completed-error:", error ?? "no error")
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        for item in self.uploadTask.items {
            if item.task?.taskIdentifier == dataTask.taskIdentifier {
                print("request base:", "-upload did receive-data:", data.JSONToStr())
...
extension XJJHTTPRequest: URLSessionDownloadDelegate {
    func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didFinishDownloadingTo location: URL) {
        ...
        for item in self.downloadTask.items {
            if item.task?.taskIdentifier == downloadTask.taskIdentifier {
                //ELog.request_log("request base:", "-download completed-", "location url:", location)
                ...
                case .temp:
                    if let urlStr = XJJFile.saveFile(from: location.path, item.fileName) {
                        ...
                        print("request base:", "-download completed-file path:", urlStr)
                    }else {
                        item.downloadResultblock?(false, nil)
                        print("request base:", "-download completed-save file failed")
                    }
                case .tmp:
    ...
    func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didWriteData bytesWritten: Int64, totalBytesWritten: Int64, totalBytesExpectedToWrite: Int64) {
        guard self.downloadTask.items.count > 0 else {return}
        //ELog.request_log("request base:", "-download-", "write bytes:", bytesWritten, "total write bytes:", totalBytesWritten, "total bytes:", totalBytesExpectedToWrite)
    }
}
...
extension XJJHTTPRequest: NSCacheDelegate {
    func cache(_ cache: NSCache<AnyObject, AnyObject>, willEvictObject obj: Any) {
        if let data = obj as? Data {
            print("request base:", "-cache-will evict data", data.count)
        }else {
            print("request base:", "-cache-will evict object", obj)
        }
    }
}
